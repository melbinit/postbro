...
function PostCardContent({ post, isMinimized }: { post: Post, isMinimized: boolean }) {
  ...
  // Handle embed error - fallback to frames
  const handleEmbedError = () => {
    console.log(`Embed failed for ${post.platform}, falling back to frames`)
      ...
      {(isMinimized || useEmbedFallback || isYouTube) && (
        <>
          {isYouTube ? (
            ...
            // Instagram/Twitter style header (only shown when using fallback or minimized)
